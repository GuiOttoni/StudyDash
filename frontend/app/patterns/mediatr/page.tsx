  ...
  return (
    ...
    >
      <div className="flex flex-col gap-8">
        ...
        {/* Pipeline visual */}
        <div>
          <h2 className="font-semibold text-white text-lg mb-4">Pipeline MediatR — fluxo de um request</h2>
                ...
                <div key={i} className="flex items-start gap-3">
                  ...
                  <div className="flex flex-col items-center shrink-0 w-5 mt-1">
                    <div className={`w-2.5 h-2.5 rounded-full shrink-0 ${
                      isHandler ? "bg-violet-500" : isBehavior ? "bg-amber-500" : "bg-zinc-600"
                    }`} />
                  ...
                  <div className={`flex flex-col gap-0.5 pb-2 ${
                    ...
                    <span className={`text-sm font-semibold ${
                      isHandler ? "text-violet-300" : isBehavior ? "text-amber-300" : "text-zinc-400"
                    }`}>{step.label}</span>
          ...
          <p className="text-zinc-600 text-xs mt-3">
            Behaviors são adicionados em <code className="bg-zinc-800 px-1 rounded">Program.cs</code> uma vez
        ...
        <div>
          <h2 className="font-semibold text-white text-lg mb-4">Comparação direta</h2>
          <div className="rounded-xl border border-zinc-800 overflow-hidden text-sm">
            <div className="grid grid-cols-3 bg-zinc-800 text-zinc-300 font-medium text-xs">
              ...
              <div className="px-4 py-2.5 border-l border-zinc-700">Hexagonal Direto</div>
              <div className="px-4 py-2.5 border-l border-zinc-700">MediatR</div>
            </div>
            {comparison.map((row, i) => (
              <div
                key={row.aspect}
                className={`grid grid-cols-3 text-xs ${i % 2 === 0 ? "bg-zinc-900" : "bg-zinc-950"}`}
              >
                <div className="px-4 py-3 text-zinc-300 font-medium">{row.aspect}</div>
                <div className="px-4 py-3 text-zinc-400 border-l border-zinc-800">{row.hexagonal}</div>
                <div className="px-4 py-3 text-zinc-400 border-l border-zinc-800">{row.mediatr}</div>
              </div>
            ))}
          </div>
        </div>
        ...
        <div className="grid grid-cols-1 sm:grid-cols-2 gap-4 text-sm">
          <div className="flex flex-col gap-3 bg-zinc-900 border border-zinc-800 rounded-xl p-4">
            <h2 className="font-semibold text-white">Quando usar Hexagonal Direto</h2>
            <ul className="space-y-2 text-zinc-400 leading-relaxed">
              <li><strong className="text-zinc-200">Apps simples</strong> — poucas use cases, dependências explícitas preferidas</li>
              <li><strong className="text-zinc-200">Rastreabilidade máxima</strong> — stack trace linear, DI transparente</li>
              <li><strong className="text-zinc-200">Sem pipeline</strong> — cross-cutting via Decorator ou sem nenhum</li>
              <li><strong className="text-zinc-200">Zero overhead</strong> — sem pacotes externos</li>
            </ul>
          </div>
          <div className="flex flex-col gap-3 bg-zinc-900 border border-zinc-800 rounded-xl p-4">
            <h2 className="font-semibold text-white">Quando adicionar MediatR</h2>
            <ul className="space-y-2 text-zinc-400 leading-relaxed">
        ...
        <div className="flex flex-col gap-3 bg-zinc-900 border border-zinc-800 rounded-xl p-5 text-sm">
          <h2 className="font-semibold text-white">Como usar os dois juntos</h2>
          <p className="text-zinc-400 leading-relaxed">
            MediatR e Hexagonal não competem — MediatR opera na camada de <strong className="text-zinc-200">roteamento e pipeline</strong>,
            enquanto Hexagonal opera na camada de <strong className="text-zinc-200">isolamento de infraestrutura</strong>.
          </p>
            ...
            {[
              ...
              <div key={layer} className={`flex flex-col gap-0.5 px-3 py-2 rounded-lg ${
                ...
                <span className={`text-xs font-semibold ${
                  color === "violet" ? "text-violet-300" : color === "amber" ? "text-amber-300" : "text-zinc-300"
                }`}>{layer}</span>
        ...
        <div className="flex flex-col gap-3">
          ...
          <p className="text-zinc-500 text-sm">
            ...
            e MediatR com <code className="text-zinc-400 bg-zinc-800 px-1 rounded text-xs">LoggingBehavior</code> +{" "}
            <code className="text-zinc-400 bg-zinc-800 px-1 rounded text-xs">ValidationBehavior</code> +
