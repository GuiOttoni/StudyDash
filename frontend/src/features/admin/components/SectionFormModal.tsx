...
import type { SectionPayload } from "@/shared/lib/api-client";
...
export function SectionFormModal({ form, editId, saving, onChange, onSave, onClose }: Props) {
  return (
      ...
      <div className="bg-[var(--bg-surface)] border border-[var(--border-strong)] rounded-md w-full max-w-lg flex flex-col gap-5 p-6 shadow-2xl">
        ...
        <div className="flex flex-col gap-4">
          <div className="grid grid-cols-2 gap-3">
            <AdminField label="Slug" value={form.slug} onChange={(v) => onChange({ ...form, slug: v })} placeholder="ex: padroes" />
            <AdminField label="Título" value={form.title} onChange={(v) => onChange({ ...form, title: v })} placeholder="ex: Padrões" />
          </div>
          <div className="grid grid-cols-2 gap-3">
            <div>
          ...
          <AdminField label="Ordem" type="number" value={String(form.order)} onChange={(v) => onChange({ ...form, order: Number(v) })} placeholder="ex: 1" />
          <div>
            <label className="text-xs text-[var(--text-tertiary)] font-medium block mb-1">Categorias (vírgula separadas)</label>
            <input
