      ...
      <div className="flex flex-col gap-6">
        ...
        <div className="grid grid-cols-1 sm:grid-cols-3 gap-4">
          {[
            ...
            <div key={type} className="flex flex-col gap-2 bg-[var(--bg-surface)] border border-[var(--border-default)] rounded-md p-4">
              <div className="flex items-center gap-2">
                <span className={`font-mono font-bold text-lg ${
                  color === "blue" ? "text-blue-400" : color === "emerald" ? "text-emerald-400" : "text-violet-400"
                }`}>{icon}</span>
                <h2 className="font-semibold text-[var(--text-primary)]">{type} Exchange</h2>
              </div>
              <p className="text-[var(--text-tertiary)] text-sm leading-relaxed">{desc}</p>
              <code className="text-xs bg-[var(--bg-surface-hover)] text-[var(--text-secondary)] px-2 py-1 rounded">{example}</code>
            </div>
          ))}
        </div>
        ...
        <div className="grid grid-cols-1 sm:grid-cols-2 gap-4 text-sm">
          <div className="flex flex-col gap-3 bg-[var(--bg-surface)] border border-[var(--border-default)] rounded-md p-4">
            <h2 className="font-semibold text-[var(--text-primary)]">RabbitMQ</h2>
            <ul className="space-y-1.5 text-[var(--text-tertiary)] leading-relaxed">
              <li>Protocolo <strong className="text-[var(--text-secondary)]">AMQP</strong> — modelo push com ACK por mensagem.</li>
              <li>Roteamento rico com exchanges, bindings e headers.</li>
              <li>Throughput ~<strong className="text-[var(--text-secondary)]">50 k msg/s</strong> por fila.</li>
          ...
          <div className="flex flex-col gap-3 bg-[var(--bg-surface)] border border-[var(--border-default)] rounded-md p-4">
            <h2 className="font-semibold text-[var(--text-primary)]">Apache Kafka</h2>
            <ul className="space-y-1.5 text-[var(--text-tertiary)] leading-relaxed">
              <li><strong className="text-[var(--text-secondary)]">Log distribuído</strong> imutável com partições.</li>
              <li>Consumer groups para escalabilidade horizontal.</li>
              <li>Throughput ~<strong className="text-[var(--text-secondary)]">1 M msg/s</strong> por partição.</li>
