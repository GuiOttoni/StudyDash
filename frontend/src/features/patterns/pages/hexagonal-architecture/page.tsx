  ...
  return (
    ...
    >
      <div className="flex flex-col gap-8">
        ...
        <div>
          <h2 className="font-semibold text-[var(--text-primary)] text-lg mb-4">Comparação: Hexagonal vs Clean vs Onion</h2>
          <div className="grid grid-cols-1 md:grid-cols-3 gap-4">
            {architectures.map((arch) => {
              const c = colorMap[arch.color];
              return (
                <div key={arch.name} className={`flex flex-col gap-3 bg-[var(--bg-surface)] border ${c.border} rounded-md p-4`}>
                  <div className="flex items-center gap-2">
                    <span className="text-2xl">{arch.icon}</span>
                    <div>
                      <h3 className={`font-bold text-base ${c.text}`}>{arch.name}</h3>
                      <p className="text-[var(--text-muted)] text-xs">{arch.author}</p>
                    </div>
                  </div>
                  ...
                  <p className="text-[var(--text-tertiary)] text-xs leading-relaxed italic">{arch.focus}</p>
        ...
        {/* Pros & Cons */}
        <div>
          <h2 className="font-semibold text-[var(--text-primary)] text-lg mb-4">Prós e Contras</h2>
          <div className="grid grid-cols-1 md:grid-cols-3 gap-4 text-sm">
            {architectures.map((arch) => {
              const c = colorMap[arch.color];
              return (
                <div key={arch.name} className="flex flex-col gap-3 bg-[var(--bg-surface)] border border-[var(--border-default)] rounded-md p-4">
        ...
        <div className="flex flex-col gap-3 bg-[var(--bg-surface)] border border-[var(--border-default)] rounded-md p-5 text-sm">
          <h2 className="font-semibold text-[var(--text-primary)] text-base">O papel central do Adapter</h2>
          <p className="text-[var(--text-tertiary)] leading-relaxed">
            Um adapter é uma classe que <strong className="text-[var(--text-primary)]">traduz a linguagem de uma tecnologia externa</strong> para
          ...
          <div className="grid grid-cols-1 sm:grid-cols-2 gap-3 mt-1">
            {[
              ...
              <div key={port} className="flex flex-col gap-2 bg-[var(--bg-surface-hover)] rounded-lg p-3">
                <code className="text-violet-300 text-xs font-mono">{port}</code>
                <ul className="space-y-1">
                  {adapters.map((a) => (
                    <li key={a} className="text-[var(--text-tertiary)] text-xs flex gap-1.5">
                      <span className="text-[var(--text-faint)]">→</span> {a}
                    </li>
                  ))}
                </ul>
              </div>
          ...
          <p className="text-[var(--text-muted)] text-xs leading-relaxed mt-1">
            O Use Case chama <code className="text-[var(--text-tertiary)] bg-[var(--bg-surface-hover)] px-1 rounded">await payment.ChargeAsync(id, amount)</code> sem saber
        ...
        <div className="grid grid-cols-1 sm:grid-cols-2 gap-4 text-sm">
          <div className="flex flex-col gap-3 bg-[var(--bg-surface)] border border-[var(--border-default)] rounded-md p-4">
            <h2 className="font-semibold text-[var(--text-primary)]">O que as três têm em comum</h2>
            <ul className="space-y-2 text-[var(--text-tertiary)] leading-relaxed">
              <li><strong className="text-[var(--text-primary)]">Dependency Inversion Principle</strong> — domínio define interfaces, infra implementa</li>
              <li><strong className="text-[var(--text-primary)]">Core isolado</strong> de frameworks, banco de dados e UI</li>
              <li><strong className="text-[var(--text-primary)]">Testabilidade</strong> via troca de implementações concretas por fakes</li>
              <li><strong className="text-[var(--text-primary)]">Evolução independente</strong> — trocar banco não afeta regras de negócio</li>
            </ul>
          </div>
          <div className="flex flex-col gap-3 bg-[var(--bg-surface)] border border-[var(--border-default)] rounded-md p-4">
            <h2 className="font-semibold text-[var(--text-primary)]">Quando escolher cada uma</h2>
            <ul className="space-y-2 text-[var(--text-tertiary)] leading-relaxed">
        ...
        <div className="flex flex-col gap-3">
          ...
          <p className="text-[var(--text-muted)] text-sm">
            Executa <code className="text-[var(--text-tertiary)] bg-[var(--bg-surface-hover)] px-1 rounded text-xs">PlaceOrderUseCase</code> com três configurações de adapters:
            pedido aprovado, pedido negado pelo limite, e swap para{" "}
            <code className="text-[var(--text-tertiary)] bg-[var(--bg-surface-hover)] px-1 rounded text-xs">AlwaysApproveAdapter</code> sem alterar o Use Case.
