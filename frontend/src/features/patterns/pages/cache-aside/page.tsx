      ...
      <div className="flex flex-col gap-6">
        ...
        <div className="grid grid-cols-1 sm:grid-cols-3 gap-4 text-sm">
          {[
            ...
            <div key={event} className="flex flex-col gap-2 bg-[var(--bg-surface)] border border-[var(--border-default)] rounded-md p-4">
        ...
        <div className="grid grid-cols-1 sm:grid-cols-2 gap-4 text-sm">
          <div className="flex flex-col gap-3 bg-[var(--bg-surface)] border border-[var(--border-default)] rounded-md p-4">
            <h2 className="font-semibold text-[var(--text-primary)]">Comparação de Estratégias</h2>
            <div className="space-y-2 text-[var(--text-tertiary)] leading-relaxed">
              <div>
                <strong className="text-[var(--text-secondary)]">Cache-Aside (Lazy)</strong>
                <p>App controla; cache só populado ao ler. Ideal para dados lidos frequentemente e atualizados raramente.</p>
              </div>
              <div>
                <strong className="text-[var(--text-secondary)]">Write-Through</strong>
                <p>App escreve banco + cache atomicamente. Cache sempre consistente; toda escrita atinge o Redis.</p>
              </div>
              <div>
                <strong className="text-[var(--text-secondary)]">Write-Behind</strong>
          ...
          <div className="flex flex-col gap-3 bg-[var(--bg-surface)] border border-[var(--border-default)] rounded-md p-4">
            <h2 className="font-semibold text-[var(--text-primary)]">Armadilhas Comuns</h2>
            <ul className="space-y-2 text-[var(--text-tertiary)] leading-relaxed">
              <li>
                ...
                <span className="block text-xs text-[var(--text-muted)] mt-0.5">Solução: mutex/lock distribuído (SETNX) ou probabilistic refresh.</span>
              </li>
              <li>
                <strong className="text-red-400">Inconsistência</strong> — update no banco sem invalidar o cache.
                <span className="block text-xs text-[var(--text-muted)] mt-0.5">Solução: sempre execute <code className="bg-[var(--bg-surface-hover)] px-1 rounded">KeyDeleteAsync</code> após writes.</span>
              </li>
              <li>
                <strong className="text-amber-400">TTL muito longo</strong> — dados stale servidos por horas.
                <span className="block text-xs text-[var(--text-muted)] mt-0.5">Solução: calibre TTL pelo ritmo de atualização do dado.</span>
              </li>
            </ul>
          </div>
        </div>
