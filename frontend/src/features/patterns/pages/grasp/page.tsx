  ...
  },
  {
    ...
    description: (
      <>
        ...
        <code className="bg-[var(--bg-surface-hover)] px-1 rounded text-sm">if/switch</code>.
  ...
  return (
    <div className="flex flex-col gap-10 max-w-4xl">
      ...
      <nav className="flex items-center gap-2 text-sm text-[var(--text-muted)]">
        ...
        <span>/</span>
        <span className="text-[var(--text-secondary)]">Princípios GRASP</span>
      </nav>

      {/* Header */}
      <div className="flex flex-col gap-4">
        <div className="flex items-center gap-3">
          <Icon name="Target" size={40} strokeWidth={1.5} className="text-[var(--text-secondary)] shrink-0" />
          <div>
            <h1 className="text-3xl font-bold text-[var(--text-primary)]">Princípios GRASP</h1>
        ...
        <div className="bg-[var(--bg-surface)] rounded-md border border-[var(--border-default)] p-6">
          ...
          <p className="text-[var(--text-tertiary)] leading-relaxed mt-2">
            <strong className="text-[var(--text-primary)]">GRASP</strong> (General Responsibility Assignment
            Software Patterns) é um conjunto de 9 princípios definidos por{" "}
            <strong className="text-[var(--text-primary)]">Craig Larman</strong> no livro{" "}
            <em>Applying UML and Patterns</em>. Diferente de padrões de implementação
            (GoF), GRASP foca em{" "}
            <strong className="text-[var(--text-primary)]">como atribuir responsabilidades</strong>{" "}
          ...
          <div className="flex flex-wrap gap-2 mt-6">
            {principles.map((p) => (
              <span key={p.abbr} className={`flex items-center gap-1.5 px-3 py-1 rounded-full border text-sm ${p.color}`}>
                <span className={`w-6 h-5 rounded text-[var(--text-primary)] text-xs font-bold flex items-center justify-center ${p.abbrBg}`}>
                  {p.abbr}
                </span>
                {p.name}
              </span>
      ...
      {principles.map((p, i) => (
        <div key={p.abbr} className="flex flex-col gap-4">
          <div className={`flex items-start gap-4 p-5 rounded-md border ${p.color}`}>
            <span className={`flex-shrink-0 w-12 h-10 rounded-lg flex items-center justify-center text-[var(--text-primary)] font-bold text-sm ${p.abbrBg}`}>
              {p.abbr}
            </span>
          ...
          <div className="rounded-md overflow-hidden border border-[var(--border-strong)] text-sm leading-relaxed">
            <div className="flex items-center justify-between px-4 py-2 bg-[var(--bg-surface-hover)] border-b border-[var(--border-strong)]">
              ...
              <span className="text-xs text-[var(--text-faint)]">{p.abbr} — {p.name}</span>
            </div>
            {renderedCodes[i] ? (
              <div
