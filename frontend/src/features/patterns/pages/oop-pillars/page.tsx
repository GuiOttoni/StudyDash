  ...
  },
  {
    ...
    description: (
      <>
        ...
        <code className="bg-[var(--bg-surface-hover)] px-1 rounded text-sm">virtual/override</code>,
...
export default function OopPillarsPage() {
  ...
  useEffect(() => {
    ...
    import("@/shared/lib/highlighter").then(({ highlightCode }) =>
      Promise.all(pillars.map((p) => highlightCode(p.code, "csharp")))
  ...
  return (
    <div className="flex flex-col gap-10 max-w-4xl">
      ...
      <nav className="flex items-center gap-2 text-sm text-[var(--text-muted)]">
        ...
        <span>/</span>
        <span className="text-[var(--text-secondary)]">4 Pilares da POO</span>
      </nav>

      {/* Header */}
      <div className="flex flex-col gap-4">
        <div className="flex items-center gap-3">
          <Icon name="BookOpen" size={40} strokeWidth={1.5} className="text-[var(--text-secondary)] shrink-0" />
          <div>
            <h1 className="text-3xl font-bold text-[var(--text-primary)]">4 Pilares da POO</h1>
        ...
        <div className="bg-[var(--bg-surface)] rounded-md border border-[var(--border-default)] p-6">
          ...
          <p className="text-[var(--text-tertiary)] leading-relaxed mt-2">
            A <strong className="text-[var(--text-primary)]">Programação Orientada a Objetos (POO)</strong> organiza
            o software em torno de objetos que encapsulam dados e comportamento. Seus quatro pilares
            fundamentais — definidos por <strong className="text-[var(--text-primary)]">Alan Kay</strong> e
            consolidados pela comunidade — são a base para código{" "}
            <strong className="text-[var(--text-primary)]">modular, reutilizável e manutenível</strong>.
          </p>
          <div className="flex flex-wrap gap-2 mt-6">
            {pillars.map((p) => (
              <span key={p.number} className={`flex items-center gap-1.5 px-3 py-1 rounded-full border text-sm ${p.color}`}>
                <span className={`w-5 h-5 rounded text-[var(--text-primary)] text-xs font-bold flex items-center justify-center ${p.numberBg}`}>
                  {p.number}
                </span>
                {p.name}
              </span>
      ...
      {pillars.map((p, i) => (
        <div key={p.number} className="flex flex-col gap-4">
          <div className={`flex items-start gap-4 p-5 rounded-md border ${p.color}`}>
            <span className={`flex-shrink-0 w-10 h-10 rounded-lg flex items-center justify-center text-[var(--text-primary)] font-bold text-xl ${p.numberBg}`}>
              {p.number}
            </span>
          ...
          <div className="rounded-md overflow-hidden border border-[var(--border-strong)] text-sm leading-relaxed">
            <div className="flex items-center justify-between px-4 py-2 bg-[var(--bg-surface-hover)] border-b border-[var(--border-strong)]">
              ...
              <span className="text-xs text-[var(--text-faint)]">{p.number} — {p.name}</span>
            </div>
            {renderedCodes[i] ? (
              <div
