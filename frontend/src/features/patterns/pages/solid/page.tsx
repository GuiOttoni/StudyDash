...
public class AreaCalculator {
  ...
  },
  {
    ...
    description: (
      <>
        Se <code className="bg-[var(--bg-surface-hover)] px-1 rounded text-sm">S</code> é subtipo de{" "}
        <code className="bg-[var(--bg-surface-hover)] px-1 rounded text-sm">T</code>, então objetos do tipo{" "}
        <code className="bg-[var(--bg-surface-hover)] px-1 rounded text-sm">T</code> podem ser substituídos por{" "}
        <code className="bg-[var(--bg-surface-hover)] px-1 rounded text-sm">S</code> sem quebrar o programa.
      </>
    ),
...
export default function SolidPage() {
  const [renderedCodes, setRenderedCodes] = useState<string[]>([]);

  useEffect(() => {
    let cancelled = false;
    import("@/shared/lib/highlighter").then(({ highlightCode }) =>
      Promise.all(principles.map((p) => highlightCode(p.code, "csharp")))
    ).then((html) => { if (!cancelled) setRenderedCodes(html); });
  ...
  return (
    <div className="flex flex-col gap-10 max-w-4xl">
      ...
      <nav className="flex items-center gap-2 text-sm text-[var(--text-muted)]">
        <Link to="/" className="hover:text-[var(--text-secondary)] transition-colors">Padrões</Link>
        <span>/</span>
        <span className="text-[var(--text-secondary)]">Princípios SOLID</span>
      </nav>

      {/* Header */}
      <div className="flex flex-col gap-4">
        <div className="flex items-center gap-3">
          <Icon name="GraduationCap" size={40} strokeWidth={1.5} className="text-[var(--text-secondary)] shrink-0" />
          <div>
            <h1 className="text-3xl font-bold text-[var(--text-primary)]">Princípios SOLID</h1>
        ...
        <div className="bg-[var(--bg-surface)] rounded-md border border-[var(--border-default)] p-6">
          ...
          <p className="text-[var(--text-tertiary)] leading-relaxed mt-2">
            SOLID é um acrônimo para cinco princípios de design orientado a objetos introduzidos por{" "}
            <strong className="text-[var(--text-primary)]">Robert C. Martin (Uncle Bob)</strong>. Seguir esses princípios
            resulta em código mais <strong className="text-[var(--text-primary)]">flexível, testável e manutenível</strong>.
          </p>
          <div className="flex flex-wrap gap-2 mt-6">
            {principles.map((p) => (
              <span key={p.letter} className={`flex items-center gap-1.5 px-3 py-1 rounded-full border text-sm ${p.color}`}>
                <span className={`w-5 h-5 rounded text-[var(--text-primary)] text-xs font-bold flex items-center justify-center ${p.letterBg}`}>
                  {p.letter}
                </span>
                {p.name}
              </span>
      ...
      {principles.map((p, i) => (
        <div key={p.letter} className="flex flex-col gap-4">
          <div className={`flex items-start gap-4 p-5 rounded-md border ${p.color}`}>
            <span className={`flex-shrink-0 w-10 h-10 rounded-lg flex items-center justify-center text-[var(--text-primary)] font-bold text-xl ${p.letterBg}`}>
              {p.letter}
            </span>
            <div className="flex flex-col gap-1">
              <h2 className="font-bold text-[var(--text-primary)] text-lg">{p.name}</h2>
              <p className="text-[var(--text-muted)] text-sm italic">{p.subtitle}</p>
              <p className="text-[var(--text-tertiary)] leading-relaxed mt-1">{p.description}</p>
          ...
          <div className="rounded-md overflow-hidden border border-[var(--border-strong)] text-sm leading-relaxed">
            <div className="flex items-center justify-between px-4 py-2 bg-[var(--bg-surface-hover)] border-b border-[var(--border-strong)]">
              <span className="text-xs text-[var(--text-muted)] uppercase tracking-wider font-medium">csharp</span>
              <span className="text-xs text-[var(--text-faint)]">{p.letter} — {p.name}</span>
            </div>
            {renderedCodes[i] ? (
              <div
                className="overflow-auto p-4 bg-[var(--bg-surface)]"
                dangerouslySetInnerHTML={{ __html: renderedCodes[i] }}
              />
            ) : (
              <pre className="overflow-auto p-4 bg-[var(--bg-surface)] text-[var(--text-secondary)]">{p.code}</pre>
            )}
          </div>
        </div>
      ))}
      ...
      <div className="flex flex-col gap-3">
        <h2 className="font-semibold text-[var(--text-primary)] text-xl">Demonstração Completa</h2>
