    ...
    >
      <div className="flex flex-col gap-6">
        <div className="flex flex-col gap-4">
          <h2 className="font-semibold text-[var(--text-primary)] text-lg">Stack</h2>
          <ul className="list-disc list-inside space-y-1 text-[var(--text-tertiary)] leading-relaxed">
            ...
            <li>Tipos: <strong className="text-[var(--text-secondary)]">int, double, bool, char, struct, enum</strong>.</li>
          </ul>
        </div>

        <div className="flex flex-col gap-4">
          <h2 className="font-semibold text-[var(--text-primary)] text-lg">Heap</h2>
          <ul className="list-disc list-inside space-y-1 text-[var(--text-tertiary)] leading-relaxed">
            <li>Alocação dinâmica via <code className="text-[var(--text-tertiary)] bg-[var(--bg-surface-hover)] px-1 rounded text-xs">new</code> — liberação gerenciada pelo GC.</li>
            <li>Cada objeto tem um <strong className="text-[var(--text-secondary)]">object header</strong> (~16 bytes de overhead).</li>
            <li>Tamanho limitado apenas pela memória disponível.</li>
            <li>Tipos: <strong className="text-[var(--text-secondary)]">class, record, string, array, delegate</strong>.</li>
          </ul>
        </div>
        ...
        <div className="flex flex-col gap-3">
          ...
          <p className="text-[var(--text-muted)] text-sm">
            Demonstra tamanhos de tipos com <code className="text-[var(--text-tertiary)] bg-[var(--bg-surface-hover)] px-1 rounded text-xs">Unsafe.SizeOf&lt;T&gt;()</code>,
            semântica de cópia e medição de alocação no heap com <code className="text-[var(--text-tertiary)] bg-[var(--bg-surface-hover)] px-1 rounded text-xs">GC.GetTotalMemory()</code>.
          </p>
          <LogRunSection
