...
export function MergeSortRunSection({ apiUrl }: Props) {
  ...
  return (
    <div className="flex flex-col gap-6">
      {/* Controls */}
      <div className="flex flex-wrap items-center gap-6 p-4 bg-[var(--bg-surface)] rounded-md border border-[var(--border-default)]">
        <div className="flex flex-col gap-2 flex-1 min-w-[180px]">
          <div className="flex items-center justify-between">
            <label className="text-sm font-medium text-[var(--text-secondary)]">
              Tamanho do array
            </label>
            <span className="text-sm font-mono font-bold text-[var(--text-primary)] bg-[var(--bg-surface-hover)] px-2 py-0.5 rounded">
              {size}
            </span>
          </div>
          <input
            ...
            className="w-full h-2 rounded-full appearance-none bg-[var(--bg-control)] accent-blue-500 cursor-pointer disabled:opacity-50 disabled:cursor-not-allowed"
          />
        ...
        <button
          ...
          className={`
            ...
            ${running
              ? "bg-[var(--bg-control)] text-[var(--text-tertiary)] cursor-not-allowed"
              : "bg-blue-600 hover:bg-blue-500 text-[var(--text-primary)] cursor-pointer shadow-lg shadow-blue-900/30"
            }
          `}
        >
          {running ? (
      ...
      <div className="flex flex-col gap-2">
        ...
        {chartState.array.length > 0 ? (
          ...
          <div className="flex items-center justify-center h-56 bg-[var(--bg-app)] rounded-md border border-[var(--border-default)] border-dashed">
            <p className="text-[var(--text-faint)] text-sm">O gráfico aparecerá aqui após executar</p>
          </div>
        )}
      </div>
