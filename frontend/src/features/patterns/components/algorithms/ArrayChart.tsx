...
export function ArrayChart({ array, comparing, sorted }: Props) {
  ...
  return (
    <div className="flex items-end justify-center gap-1.5 h-56 px-4 pt-4 pb-2 bg-[var(--bg-app)] rounded-md border border-[var(--border-default)]">
      {array.map((value, i) => {
        ...
        return (
          <div key={i} className="flex flex-col items-center gap-1 flex-1 min-w-0">
            <span className={`text-xs font-mono font-semibold transition-colors ${isComparing ? "text-orange-300" : isSorted ? "text-emerald-400" : "text-[var(--text-tertiary)]"}`}>
              {value}
            </span>
            <div
            ...
            />
            <span className="text-[10px] text-[var(--text-faint)] font-mono">{i}</span>
          </div>
        );
      })}
    </div>
